...
import { Snowflake, Map as MapIcon, Thermometer, Navigation, Sun, Moon, CloudRain, CloudSnow, Cloud } from 'lucide-react';
...
export default function Home() {
  ...
  const t = {
    sr: { map: "Mapa", close: "Zatvori", snow: "Očekivani Sneg", precip: "Ukupne padavine", rain: "Moguća kiša" },
    en: { map: "Map", close: "Close", snow: "Expected Snow", precip: "Total Precipitation", rain: "Possible Rain" }
  }[lang];

  return (
    <div className="min-h-screen transition-colors bg-white dark:bg-[#020617]">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <button onClick={() => setShowMap(!showMap)} className="w-full mb-8 py-5 rounded-3xl font-black uppercase tracking-widest flex items-center justify-center gap-2 dark:text-white bg-white dark:bg-white/5 border border-black/5 shadow-sm hover:bg-slate-50 transition-colors">
          <MapIcon size={20} /> {showMap ? t.close : t.map}
        </button>
        ...
        <div className="flex justify-center mb-16 overflow-x-auto pb-4 no-scrollbar">
           <div className="p-1.5 rounded-full flex gap-1 bg-slate-100/50 dark:bg-white/5 border border-black/5">
             {[6, 12, 24, 48, 72, 120, 168, 240].map(v => (
               <button key={v} onClick={() => setTimeframe(v)} className={`px-6 py-3 rounded-full text-[10px] font-black uppercase transition-all ${timeframe === v ? 'bg-[#3b82f6] text-white shadow-lg scale-105' : 'opacity-40 hover:opacity-100 dark:text-white'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {!loading && resorts.map((resort) => {
            ...
            return (
              <Link key={resort.id} href={`/resort/${resort.id}`} className="p-8 rounded-[4rem] block bg-white dark:bg-[#0f172a] shadow-xl hover:-translate-y-2 transition-all duration-500 group border border-black/[0.03] dark:border-white/[0.03]">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest text-white mb-4" style={{ backgroundColor: s.color }}>
                  {s.icon} {s.txt}
                </div>
                <h3 className="text-3xl font-black uppercase tracking-tighter dark:text-white mb-1">{resort.name}</h3>
                <p className="text-[10px] font-bold uppercase opacity-30 dark:text-white mb-6 italic tracking-widest">{resort.country}</p>
                
                <div className="h-44 rounded-[3.5rem] flex flex-col justify-center items-center text-white relative overflow-hidden mb-8 shadow-inner" style={{ backgroundColor: s.color }}>
                  ...
                  <table className="w-full text-[11px] font-black uppercase tracking-widest text-black dark:text-white">
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="py-3 text-right italic font-black">{rain.toFixed(1)}mm</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
