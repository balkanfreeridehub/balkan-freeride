...
export default function Home() {
  ...
  const [resorts, setResorts] = useState<any[]>([]); // Ovo sprečava ReferenceError
    ...
    <div className="min-h-screen bg-white dark:bg-[#020617] text-slate-900 dark:text-slate-100 transition-colors duration-500">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? (
            <div className="col-span-full text-center py-20 font-black uppercase italic opacity-50 tracking-widest animate-pulse">
              {t.loading}
            </div>
          ) : (
            resorts.map((resort) => {
              ...
              return (
                <div key={resort.id} className="bg-slate-50 dark:bg-white/5 border dark:border-white/10 p-8 rounded-[3rem] hover:shadow-2xl transition-all group">
                  ...
                  <p className="text-[10px] font-bold text-blue-500 uppercase mb-6 tracking-widest">{translateCondition(resort.condition)}</p>
                  ...
                  <div className="mb-8 bg-blue-600 p-8 rounded-[2.5rem] text-white relative overflow-hidden shadow-xl shadow-blue-600/30">
                    <div className="relative z-10">
                      <p className="text-[10px] font-black uppercase opacity-70 mb-1">{t.forecast} (+{timeframe}h)</p>
                      <p className="text-5xl font-black italic">+{snowVal} <span className="text-2xl uppercase">cm</span></p>
                    </div>
                  </div>

                  <button onClick={() => setSelectedResort(resort)} className="w-full py-5 bg-slate-900 dark:bg-white text-white dark:text-black font-black uppercase text-[10px] tracking-[0.2em] rounded-2xl hover:bg-blue-600 dark:hover:bg-blue-500 transition-all">
                    {t.cam}
                  </button>
                </div>
              );
