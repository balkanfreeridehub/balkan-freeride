...
const FlagUSA = () => <svg viewBox="0 0 36 24" className="w-5 h-5 rounded-full shadow-sm"><path fill="#fff" d="M0 0h36v24H0z"/><path fill="#B22234" d="M0 0h36v1.8H0z"/><path fill="#3C3B6E" d="M0 0h14.4v12.6H0z"/></svg>;
...
// SKELETON KARTICA (Prikazuje se dok se učitava)
const SkeletonCard = () => (
  <div className="bg-white dark:bg-white/5 p-8 flex flex-col h-[500px] shadow-xl ring-1 ring-black/5 animate-pulse rounded-sm">
    <div className="w-24 h-6 bg-slate-200 dark:bg-white/10 rounded-full mb-5"></div>
    <div className="w-48 h-10 bg-slate-200 dark:bg-white/10 rounded mb-2"></div>
    <div className="w-20 h-4 bg-slate-100 dark:bg-white/5 rounded mb-8"></div>
    <div className="w-full h-44 bg-slate-200 dark:bg-white/10 rounded-[2rem] mb-8"></div>
    <div className="grid grid-cols-3 gap-3">
      <div className="h-24 bg-slate-100 dark:bg-white/5 rounded-[1.5rem]"></div>
      <div className="h-24 bg-slate-100 dark:bg-white/5 rounded-[1.5rem]"></div>
      <div className="h-24 bg-slate-100 dark:bg-white/5 rounded-[1.5rem]"></div>
    </div>
  </div>
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] text-slate-900 dark:text-slate-100 font-sans pb-20">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className={`overflow-hidden transition-all duration-700 ease-in-out ${showMap ? 'max-h-[600px] mb-12 opacity-100' : 'max-h-0 opacity-0'}`}>
           <div className="h-[550px] shadow-2xl ring-1 ring-black/10 dark:ring-white/10 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-900">
              {/* Mapa dobija sopstveni loading unutar komponente ako treba, ali ovde je renderujemo normalno */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {loading ? (
            // Prikazujemo 6 skeleton kartica dok traje loading
            ...
            resorts.map((resort) => {
              ...
              return (
                <div key={resort.id} className="cursor-pointer bg-white dark:bg-white/5 p-8 flex flex-col shadow-xl ring-1 ring-black/5 dark:ring-white/5 hover:translate-y-[-8px] transition-all duration-500 group">
                  ...
                  <div className="mt-6 text-right text-[9px] font-black uppercase text-slate-300 group-hover:text-blue-600 transition-colors tracking-widest">Detalji & Kamere →</div>
                </div>
              );
            })
          )}
