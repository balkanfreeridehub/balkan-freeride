      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="flex justify-center mb-16 overflow-x-auto py-2">
           <div className="bg-white dark:bg-white/5 p-2 rounded-full border border-black/5 flex gap-1">
             {[6, 12, 24, 48, 72, 120, 168, 240].map(v => (
               <button key={v} onClick={() => setTimeframe(v)} className={`px-6 py-3 rounded-full text-[10px] font-black uppercase transition-all ${timeframe === v ? 'bg-[#A855F7] text-white shadow-lg' : 'hover:bg-slate-100 dark:hover:bg-white/10 dark:text-white'}`}>
                 {v < 24 ? `${v}h` : `${v/24}d`}
               </button>
             ))}
           </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {resorts.map((resort) => {
            ...
            return (
              <Link key={resort.id} href={`/resort/${resort.id}`} className="group bg-white dark:bg-white/5 p-8 rounded-[3.5rem] border border-black/5 hover:shadow-2xl transition-all duration-500 block">
                ...
                <div className="h-64 rounded-[3rem] p-8 flex flex-col justify-between text-white relative overflow-hidden shadow-xl" style={{ backgroundColor: s.color }}>
                   <div className="flex items-baseline justify-center gap-1">
                      <span className="text-[6rem] font-black tracking-tighter">{snow.toFixed(0)}</span>
