...
const translations = {
  sr: { hub: "Freeride", live: "UŽIVO KAMERE", snow: "Prognoza snega", total: "Ukupno", rain: "Kiša", scanning: "Skeniranje Balkana...", loading: "Učitavanje..." },
  en: { hub: "Freeride", live: "LIVE CAMS", snow: "Snow Forecast", total: "Total", rain: "Rain", scanning: "Scanning Balkans...", loading: "Loading..." }
};
...
export default function Home() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      // Pozivamo sa indexom za "staggered" loading (sprečava 429 error)
      ...
      <nav className="sticky top-0 bg-white/70 dark:bg-[#020617]/70 backdrop-blur-xl z-50 px-6 py-4 shadow-sm ring-1 ring-black/5 dark:ring-white/5">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setLang(lang === 'sr' ? 'en' : 'sr')} className="text-[11px] font-black px-4 py-2 bg-white dark:bg-white/5 rounded-2xl shadow-sm ring-1 ring-black/5 dark:ring-white/10 hover:bg-slate-50 transition-all italic">
              {lang === 'sr' ? 'SRB' : 'ENG'}
            </button>
            <ThemeToggle />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {loading ? (
            ...
            resorts.map((resort) => {
              ...
              return (
                <div key={resort.id} className="group bg-white dark:bg-white/5 p-8 rounded-[3.5rem] flex flex-col h-full shadow-xl shadow-slate-200/50 dark:shadow-none ring-1 ring-black/5 dark:ring-white/5 hover:translate-y-[-8px] transition-all duration-500">
                  <div className="mb-6">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-10 text-center">
                    ...
                    <div className="bg-slate-50 dark:bg-white/5 rounded-[2.2rem] h-28 flex flex-col items-center justify-center">
                      ...
                      <span className="text-xl font-black tracking-tighter">{resort.current?.windSpeed ?? '--'}<span className="text-[10px] font-normal opacity-40 ml-0.5 uppercase">m/s</span></span>
                    </div>
                  </div>
