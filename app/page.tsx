...
import { Snowflake, Map as MapIcon, Thermometer, Wind, Rocket, Zap, CircleSlash, Navigation, Cloud, Sun, CloudRain, CloudSnow, Moon, CloudLightning } from 'lucide-react';

// Mapu učitavamo dinamički da ne pravi probleme sa SSR
const BalkanMap = dynamic(() => import('@/components/BalkanMap'), { 
  ssr: false,
  loading: () => <div className="h-[550px] w-full bg-slate-100 animate-pulse rounded-[3.5rem]" />
});
...
export default function Home() {
  ...
  const getWeatherIcon = (code: number) => {
    ...
    if (code === undefined) return <Sun size={28} className="opacity-20" />;
    ...
    if (code >= 45) return <Cloud size={28} className="text-slate-400" />;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] pb-20 font-sans">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <button onClick={() => setShowMap(!showMap)} className="w-full mb-6 py-6 bg-white dark:bg-white/5 border border-black/5 rounded-[2.5rem] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-2 transition-all hover:bg-slate-50">
            ...
            resorts.map((resort) => {
              ...
              return (
                <Link key={resort.id} href={`/resort/${resort.id}`} className="group bg-white dark:bg-white/5 p-10 rounded-[4rem] border border-black/5 hover:shadow-2xl transition-all duration-500 block">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-20 dark:text-white mt-10 mb-4 text-center">Current Conditions</p>
