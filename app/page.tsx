          ...
          {resorts.map((resort) => {
            ...
            if (resort.hourly) {
              for (let i = 0; i < timeframe; i++) {
                ...
                if (p > 0) {
                  totalP += p;
                  tv <= 1 ? calcSnow += p * 1.2 : calcRain += p;
                }
              }
            }
              ...
              <div key={resort.id} onClick={() => router.push(`/resort/${resort.id}`)} className="bg-white dark:bg-white/5 p-10 flex flex-col shadow-xl border border-black/5 dark:border-white/5 rounded-[3rem] cursor-pointer hover:-translate-y-2 transition-all group">
                ...
                <div className={`h-48 p-8 rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl mb-8 flex flex-col justify-center ${calcSnow >= 100 ? 'bg-red-600' : 'bg-blue-600'}`}>
                  <div className="flex items-baseline font-black italic tracking-tighter tabular-nums leading-none">
                    ...
                    <span className="text-xl ml-2 opacity-50 not-italic uppercase font-bold">cm</span>
                  </div>
                  <div className="mt-4 pt-4 border-t border-white/10 flex gap-4 text-[10px] font-black uppercase tracking-widest opacity-80">
                    <span className="flex items-center gap-1"><Droplets size={12}/> {totalP.toFixed(1)}mm</span>
                    {calcRain > 0 && <span className="text-red-200">! {calcRain.toFixed(1)}mm Rain</span>}
                  </div>
                </div>

                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 ml-2">Current Weather</p>
