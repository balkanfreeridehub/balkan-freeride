...
import { 
  ...
  CloudFog, Zap, Snowflake, CircleSlash, Rocket, Droplets, Info, Star, Map as MapIcon, ChevronUp
} from 'lucide-react';
...
const translations = {
  sr: { hub: "Freeride Hub", live: "UŽIVO KAMERE", snow: "Prognoza snega", total: "Ukupno", rain: "Kiša", scanning: "Skeniranje...", mapShow: "Prikaži mapu", mapHide: "Sakrij mapu" },
  en: { hub: "Freeride Hub", live: "LIVE CAMS", snow: "Snow Forecast", total: "Total", rain: "Rain", scanning: "Scanning...", mapShow: "Show Map", mapHide: "Hide Map" }
};

const FlagSRB = () => <svg viewBox="0 0 36 24" className="w-5 h-5 rounded-full"><path fill="#C6363C" d="M0 0h36v8H0z"/><path fill="#fff" d="M0 8h36v8H0z"/><path fill="#2D4D8F" d="M0 16h36v8H0z"/></svg>;
const FlagUSA = () => <svg viewBox="0 0 36 24" className="w-5 h-5 rounded-full"><path fill="#fff" d="M0 0h36v24H0z"/><path fill="#B22234" d="M0 0h36v1.8H0zm0 3.6h36v1.8H0zm0 3.6h36v1.8H0zm0 3.6h36v1.8H0zm0 3.6h36v1.8H0zm0 3.6h36v1.8H0z"/><path fill="#3C3B6E" d="M0 0h14.4v12.6H0z"/></svg>;
...
export default function Home() {
  ...
  const getStatus = (snow: number) => {
    ...
    return { cls: 'bg-slate-300 dark:bg-slate-700 text-slate-500', txt: 'SKIP', icon: <CircleSlash className="w-3.5 h-3.5" /> };
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] text-slate-900 dark:text-slate-100 font-sans transition-colors duration-500">
      <nav className="sticky top-0 bg-white/80 dark:bg-[#020617]/80 backdrop-blur-xl z-50 px-6 py-4 ring-1 ring-black/5 dark:ring-white/5">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <h1 className="text-xl font-black uppercase tracking-tighter">
            Balkan <span className="text-blue-600 italic">{t.hub}</span>
          </h1>
          <div className="flex items-center gap-6">
            <button onClick={() => setLang(lang === 'sr' ? 'en' : 'sr')} className="hover:scale-110 transition-transform">{lang === 'sr' ? <FlagUSA /> : <FlagSRB />}</button>
            <ThemeToggle />
          </div>
        </div>
      </nav>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <button 
          ...
          className="w-full mb-8 py-4 bg-white dark:bg-white/5 ring-1 ring-black/5 dark:ring-white/10 flex items-center justify-center gap-3 font-black uppercase text-[12px] tracking-widest hover:bg-slate-50 dark:hover:bg-white/10 transition-all shadow-sm"
        >
        ...
        {showMap && (
          <div className="overflow-hidden mb-12 shadow-2xl h-[500px] bg-white dark:bg-white/5 ring-1 ring-black/10 dark:ring-white/10 animate-in fade-in slide-in-from-top-4 duration-500">
              ...
              <div key={resort.id} className="bg-white dark:bg-white/5 p-8 flex flex-col h-full shadow-xl ring-1 ring-black/5 dark:ring-white/5 hover:translate-y-[-5px] transition-all duration-500 group">
                ...
                <div className="h-44 bg-blue-600 p-8 rounded-[2rem] text-white relative overflow-hidden shadow-xl mb-8 flex flex-col justify-center">
                  ...
                  <div className="flex gap-4 mt-4 pt-4 border-t border-white/20 text-[11px] font-black uppercase">
                    ...
                    {calcRain > 0 && <span className="text-red-300">!! Kiša: {calcRain.toFixed(1)}mm</span>}
                  </div>
                </div>

                {/* TRI BOXA - UNIFIKOVAN SHADOW-INNER */}
                <div className="grid grid-cols-3 gap-3 mb-8 text-center">
                  <div className="bg-slate-50 dark:bg-white/5 rounded-[1.5rem] h-24 flex items-center justify-center shadow-inner ring-1 ring-black/[0.02] dark:ring-white/[0.05]">
                    <CloudFog className="w-8 h-8 opacity-20" /> {/* Primer ikonice */}
                  </div>
                  <div className="bg-slate-50 dark:bg-white/5 rounded-[1.5rem] h-24 flex flex-col items-center justify-center shadow-inner ring-1 ring-black/[0.02] dark:ring-white/[0.05]">
                ...
                <button onClick={() => setSelectedResort(resort)} className="mt-auto w-full py-6 bg-slate-900 dark:bg-white text-white dark:text-black font-black uppercase text-[11px] tracking-[0.3em] rounded-[1.2rem] hover:bg-blue-600 dark:hover:bg-blue-600 dark:hover:text-white transition-all shadow-lg active:scale-95 italic uppercase">
                  {t.live}
                </button>
              </div>
            );
