...
import { Snowflake, Map as MapIcon, ChevronUp, Droplets, Wind, Thermometer, CloudFog, Zap, Rocket, Star, CircleSlash } from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const allData = await getAllWeatherData(balkanResorts);
        setResorts(balkanResorts.map((r, i) => ({ ...r, ...allData[i] })));
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] pb-20">
      <nav className="sticky top-0 bg-white/80 dark:bg-[#020617]/80 backdrop-blur-xl z-50 px-8 py-5 border-b border-black/5">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <button onClick={() => setShowMap(!showMap)} className="w-full mb-10 py-6 bg-white dark:bg-white/5 border border-black/5 rounded-3xl font-black uppercase tracking-widest flex items-center justify-center gap-2">
          <MapIcon className="text-blue-600" /> {showMap ? 'Hide Map' : 'Show Map'}
        </button>

        {showMap && (
          <div className="h-[600px] mb-20 rounded-[3rem] overflow-hidden border border-black/5 bg-slate-100 relative shadow-2xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {loading ? (
            [...Array(6)].map((_, i) => (
              <div key={i} className="bg-white dark:bg-white/5 p-10 rounded-[3.5rem] animate-pulse">
                <div className="w-24 h-6 bg-slate-200 dark:bg-slate-800 rounded-full mb-6" />
                <div className="w-full h-48 bg-slate-100 dark:bg-slate-800 rounded-[2.8rem] mb-8" />
                <div className="grid grid-cols-3 gap-4">
                  <div className="h-24 bg-slate-50 dark:bg-slate-800 rounded-[2.2rem]" />
                  <div className="h-24 bg-slate-50 dark:bg-slate-800 rounded-[2.2rem]" />
                  <div className="h-24 bg-slate-50 dark:bg-slate-800 rounded-[2.2rem]" />
                </div>
              </div>
            ...
            resorts.map((resort) => {
              let snow = 0;
              resort.hourly?.precipitation?.forEach((p:any, i:number) => {
                if (i < timeframe && p > 0 && resort.hourly.temperature_2m[i] <= 1) snow += p * 1.2;
              });
              const s = getStatus(snow);

              return (
                <div key={resort.id} onClick={()=>router.push(`/resort/${resort.id}`)} className="bg-white dark:bg-white/5 p-10 rounded-[3.5rem] border border-black/5 cursor-pointer hover:-translate-y-2 transition-all shadow-xl">
                  <div className={`inline-flex px-4 py-1 rounded-full text-[10px] font-black mb-6 ${s.cls}`}>{s.txt}</div>
                  <h3 className="text-4xl font-black uppercase tracking-tighter mb-8">{resort.name}</h3>
                  <div className="h-48 p-8 rounded-[2.8rem] text-white flex flex-col justify-center" style={{backgroundColor: s.color}}>
                    <span className="text-6xl font-black italic">+{snow.toFixed(1)}<span className="text-sm not-italic ml-2 uppercase">cm</span></span>
                  </div>
                  <div className="grid grid-cols-3 gap-4 mt-8">
                     <div className="h-24 bg-slate-50 dark:bg-white/5 rounded-[2.2rem] flex flex-col items-center justify-center font-bold">
                        <Thermometer size={14} className="mb-1 opacity-40"/><span className="text-2xl">{resort.current?.temperature_2m}°</span>
                     </div>
                     <div className="h-24 bg-slate-50 dark:bg-white/5 rounded-[2.2rem] flex flex-col items-center justify-center font-bold">
                        <Wind size={14} className="mb-1 text-blue-500"/><span className="text-2xl">{resort.current?.wind_speed_10m}</span>
                     </div>
                     <div className="h-24 bg-slate-50 dark:bg-white/5 rounded-[2.2rem] flex flex-col items-center justify-center opacity-30"><CloudFog size={24}/></div>
                  </div>
                </div>
              );
            })
