...
// Dinamički import koji čuva tvoj stari dizajn mape
const BalkanMap = dynamic(() => import('../components/BalkanMap'), { 
  ssr: false,
  loading: () => <div className="h-full w-full bg-slate-50 dark:bg-slate-900 animate-pulse flex items-center justify-center font-black opacity-20 italic uppercase">Učitavam mapu...</div>
});
...
const translations = {
  sr: { snowfall: "Sneg", total: "Ukupno", rain: "Kiša", cams: "UŽIVO KAMERE", scan: "SKENIRANJE..." },
  en: { snowfall: "Snowfall", total: "Total", rain: "Rain", cams: "LIVE CAMS", scan: "SCANNING..." }
};

const WeatherVisual = ({ code }: { code: number }) => {
  const hour = new Date().getHours();
  const isNight = hour >= 19 || hour < 6;
  const className = "w-7 h-7 text-slate-800 dark:text-white"; 
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#020617] text-slate-900 dark:text-slate-100 transition-colors duration-500 font-sans italic">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        {/* TVOJA STARA MAPA SA FULL ŠIRINOM */}
        ...
        <div className="flex flex-wrap justify-center gap-1.5 mb-12 p-1.5 bg-slate-100 dark:bg-white/5 rounded-2xl w-fit mx-auto border dark:border-white/5 shadow-inner font-black">
          {timeOptions.map((opt) => (
            <button key={opt.value} onClick={() => setTimeframe(opt.value)}
              className={`px-4 py-2 rounded-xl text-[10px] uppercase transition-all ${timeframe === opt.value ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-blue-500'}`}>
              {opt.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {loading ? <div className="col-span-full text-center py-20 font-black uppercase opacity-20 animate-pulse tracking-widest">{t.scan}</div> : 
            resorts.map((resort) => {
              ...
              return (
                <div key={resort.id} className="bg-slate-50 dark:bg-white/5 border dark:border-white/10 p-7 rounded-[3rem] shadow-sm flex flex-col h-full hover:shadow-xl transition-all">
                  <div className="h-20 mb-4">
                    ...
                    <h3 className="text-2xl font-black uppercase leading-none tracking-tight">{resort.name}</h3>
                  ...
                  <div className="h-44 bg-blue-600 p-8 rounded-[2.5rem] text-white relative overflow-hidden shadow-xl shadow-blue-600/30 mb-6 flex flex-col justify-center">
                    <div className="relative z-10">
                      <p className="text-[10px] font-black uppercase opacity-70 mb-1 tracking-widest">{t.snowfall} (+{timeframe < 24 ? timeframe + 'h' : timeframe/24 + 'd'})</p>
                      <p className="text-5xl font-black mb-2 tracking-tighter">+{calcSnow.toFixed(1)} <span className="text-2xl font-normal opacity-50 uppercase ml-1 tracking-normal">cm</span></p>
                  ...
                  {/* VRAĆENE KOLONE - SAVRŠENA SIMETRIJA */}
                  <div className="grid grid-cols-3 gap-3 mb-8">
                    ...
                    <div className="bg-white dark:bg-white/5 rounded-[2rem] border dark:border-white/5 h-24 flex flex-col items-center justify-center shadow-sm px-1">
                      <Thermometer className="w-5 h-5 mb-1 text-slate-300" />
                      <span className="text-xl font-black">{resort.current.temp}°C</span>
                    </div>
                    ...
                    <div className="bg-white dark:bg-white/5 rounded-[2rem] border dark:border-white/5 h-24 flex flex-col items-center justify-center shadow-sm px-1">
                      ...
                      />
                      <span className="text-xl font-black tracking-tighter">{resort.current.windSpeed}<span className="text-[9px] opacity-30 ml-0.5">m/s</span></span>
                    </div>
                  </div>
