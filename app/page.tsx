...
const timeOptions = [
  ...
  { label: '72h', value: 72 }, { label: '7D', value: 168 }
];
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <section className="mb-8 rounded-[2rem] overflow-hidden border border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900">
          ...
          {timeOptions.map((opt) => (
            <button
              ...
              className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                ...
                : 'hover:bg-white dark:hover:bg-slate-800 opacity-50'
              }`}
            >
              {opt.label}
            </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {resortsWithData.map((resort) => (
            <div key={resort.id} className="bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-white/5 p-8 rounded-[2.5rem] shadow-sm hover:shadow-2xl transition-all">
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h3 className="text-2xl font-black uppercase italic leading-none">{resort.name}</h3>
                  <p className="text-[10px] font-bold text-blue-500 mt-2 uppercase tracking-widest">{resort.condition}</p>
                </div>
