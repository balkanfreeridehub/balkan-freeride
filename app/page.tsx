...
import { Snowflake, Map as MapIcon, Thermometer, Wind, CloudRain, Zap, Rocket, CircleSlash, Navigation, Cloud, Sun, CloudSnow } from 'lucide-react';
...
export default function Home() {
  ...
  // TVOJE ORIGINALNE BOJE I KATEGORIJE
  ...
  const getWeatherIcon = (code: number) => {
    if (code >= 71) return <CloudSnow className="text-blue-400" />;
    if (code >= 51) return <CloudRain className="text-blue-400" />;
    if (code >= 1) return <Cloud className="text-slate-400" />;
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] pb-20 font-sans">
      <nav className="sticky top-0 bg-white/80 dark:bg-[#020617]/80 backdrop-blur-xl z-50 px-8 py-5 border-b border-black/5 flex justify-between items-center">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo(0,0)}>
          ...
          <h1 className="text-2xl font-black italic uppercase tracking-tighter">
            Balkan<span className="text-[#f59e0b]">Freeride</span> <span className="text-blue-600 italic ml-1">Hub</span>
          </h1>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <button onClick={() => setShowMap(!showMap)} className="w-full mb-6 py-6 bg-white dark:bg-white/5 border border-black/5 rounded-[2.5rem] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-2">
          <MapIcon size={20} className="text-blue-600" /> {showMap ? 'Hide Map' : 'Show Map'}
        </button>

        {showMap && (
          <div className="h-[500px] mb-10 rounded-[3rem] overflow-hidden border border-black/5 shadow-2xl">
        ...
        <div className="flex justify-center mb-16 overflow-x-auto py-2">
          <div className="bg-white dark:bg-white/5 p-2 rounded-full border border-black/5 flex gap-1">
            {TIMES.map(t => (
              <button key={t.v} onClick={() => setTimeframe(t.v)} className={`px-6 py-3 rounded-full text-[10px] font-black uppercase transition-all ${timeframe === t.v ? 'bg-blue-600 text-white shadow-lg' : 'hover:bg-slate-100 dark:hover:bg-white/10'}`}>
                {t.l}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {loading ? (
             [...Array(6)].map((_, i) => (
              <div key={i} className="h-[500px] bg-white dark:bg-white/5 rounded-[4rem] p-10 border border-black/5 relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-slate-100/50 dark:via-white/5 to-transparent -translate-x-full animate-[shimmer_2s_infinite]" />
                <div className="w-32 h-8 bg-slate-200 dark:bg-slate-800 rounded-full mb-8" />
                <div className="w-full h-40 bg-slate-100 dark:bg-slate-800/50 rounded-[3rem] mb-10" />
              </div>
            ))
          ) : (
            resorts.map((resort) => {
              ...
              return (
                <div key={resort.id} onClick={()=>router.push(`/resort/${resort.id}`)} className="group bg-white dark:bg-white/5 p-10 rounded-[4rem] border border-black/5 cursor-pointer hover:shadow-2xl hover:-translate-y-4 transition-all duration-500">
                  ...
                  <div className="h-44 rounded-[3rem] flex flex-col items-center justify-center mb-10 text-white" style={{ backgroundColor: s.color }}>
                    <span className="text-7xl font-black italic">+{snow.toFixed(1)}</span>
                    <span className="text-[10px] font-black uppercase tracking-widest opacity-60">cm snow</span>
                  </div>

                  <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 ml-2">Current Status</div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 dark:bg-white/5 p-5 rounded-[2rem] flex flex-col items-center">
                       <Thermometer size={14} className="mb-2 opacity-30" />
                       <span className="text-xl font-black">{resort.current?.temperature_2m}°</span>
                    </div>
                    <div className="bg-slate-50 dark:bg-white/5 p-5 rounded-[2rem] flex flex-col items-center">
                       <CloudRain size={14} className={`mb-2 ${rain > 0 ? 'text-blue-500' : 'opacity-30'}`} />
                       <span className="text-xl font-black">{rain.toFixed(1)}<span className="text-[10px] ml-1">mm</span></span>
                    </div>
                    <div className="bg-slate-50 dark:bg-white/5 p-5 rounded-[2rem] flex flex-col items-center col-span-2">
                       <div className="flex items-center gap-4">
                         <Wind size={14} className="opacity-30" />
                         <span className="text-xl font-black">{resort.current?.wind_speed_10m} <span className="text-[10px]">kmh</span></span>
                         <Navigation size={14} style={{ transform: `rotate(${resort.current?.wind_direction_10m}deg)` }} className="text-blue-500" />
                       </div>
                    </div>
                  </div>
                </div>
