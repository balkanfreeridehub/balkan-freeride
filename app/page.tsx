...
const translations = {
  sr: { title: "Balkan Freeride Hub", showMap: "Prikaži Mapu", closeMap: "Zatvori Mapu", total: "Ukupno", rain: "Kiša!" },
  en: { title: "Balkan Snow Hub", showMap: "Show Map", closeMap: "Close Map", total: "Total", rain: "Rain Alert" }
}
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] text-slate-900 dark:text-slate-100 font-sans pb-20">
      <nav className="sticky top-0 bg-white/80 dark:bg-[#020617]/80 backdrop-blur-xl z-50 px-8 py-5 border-b border-black/5 dark:border-white/5">
        <div className="max-w-7xl mx-auto flex justify-between items-center font-black uppercase tracking-tighter">
          <h1 className="text-2xl italic">{t.title}</h1>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <button onClick={() => setShowMap(!showMap)} className="w-full mb-10 py-6 bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 flex items-center justify-center gap-4 font-black uppercase text-[12px] tracking-[0.3em] rounded-3xl hover:bg-slate-50 transition-all">
          ...
          {showMap ? t.closeMap : t.showMap}
        </button>

        <div className={`transition-all duration-1000 ${showMap ? 'max-h-[700px] mb-24 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
           <div className="h-[600px] shadow-2xl border border-black/5 dark:border-white/10 bg-slate-100 dark:bg-slate-900 overflow-hidden relative rounded-[3rem]">
              <BalkanMap resorts={resorts} timeframe={timeframe} onSelect={(r) => setSelectedResort(r)} />
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
          {resorts.map((resort) => {
            ...
            return (
              <div key={resort.id} onClick={() => setSelectedResort(resort)} className="bg-white dark:bg-white/5 p-10 flex flex-col shadow-xl border border-black/5 dark:border-white/5 rounded-[3rem] cursor-pointer hover:-translate-y-2 transition-all group">
                <div className="mb-8">
                ...
                <div className={`h-48 p-8 rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl mb-8 flex flex-col justify-center ${calcSnow >= 100 ? 'bg-red-600' : 'bg-blue-600'}`}>
                  ...
                  <div className="mt-6 pt-4 border-t border-white/10 flex gap-4 text-[10px] font-black uppercase">
                     {calcRain > 0 && <span className="text-red-300 animate-pulse">! {calcRain.toFixed(1)}mm {t.rain}</span>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                   ...
                   <div className="bg-slate-50 dark:bg-white/5 rounded-[2rem] h-24 flex flex-col items-center justify-center">
      ...
      {selectedResort && <ResortDetails resort={selectedResort} lang={lang} onClose={() => setSelectedResort(null)} />}
    </div>
  );
}