...
const FlagSRB = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 64 64"><path fill="#ed4c5c" d="M32 2C18.9 2 7.8 10.4 3.7 22h56.6C56.2 10.4 45.1 2 32 2"/><path fill="#f9f9f9" d="M32 62c13.1 0 24.2-8.3 28.3-20H3.7C7.8 53.7 18.9 62 32 62"/><path fill="#2872a0" d="M3.7 22C2.6 25.1 2 28.5 2 32s.6 6.9 1.7 10h56.6c1.1-3.1 1.7-6.5 1.7-10s-.6-6.9-1.7-10z"/><path fill="#ed4c5c" d="M15.5 21.7v16.2C15.5 43.5 20.2 48 26 48s10.5-4.5 10.5-10.1V21.7z"/></svg>;
...
export default function Home() {
  ...
  const getStatus = (snow: number) => {
    ...
    if (snow >= 40) return { color: '#8b57ff', txt: 'DEEP POWDER', icon: <Snowflake size={14}/> };
  ...
  const getWeatherIcon = (code: number) => {
    ...
    const props = { size: 36, strokeWidth: 2, className: "drop-shadow-lg" };
    if (code >= 71) return <CloudSnow {...props} className="text-white dark:text-blue-100" />;
    if (code >= 51) return <CloudRain {...props} className="text-blue-400" />;
      ...
      <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#020617] pb-20 font-sans transition-colors">
        ...
        <nav className="sticky top-0 bg-white/80 dark:bg-[#020617]/80 backdrop-blur-xl z-50 px-8 py-5 border-b border-black/5 flex justify-between items-center">
          <Link href="/" className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#8b57ff] rounded-xl flex items-center justify-center text-white shadow-lg shadow-[#8b57ff]/20"><Snowflake size={24}/></div>
            <h1 className="text-2xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Balkan<span className="text-[#8b57ff] ml-1">Freeride</span></h1>
          </Link>
          
          <div className="flex items-center gap-6">
            <button onClick={() => setIsDark(!isDark)} className="p-2 bg-slate-100 dark:bg-white/5 rounded-full text-slate-500 dark:text-yellow-400 hover:scale-110 transition-all cursor-pointer">
        ...
        <main className="max-w-7xl mx-auto px-6 py-10">
          ...
          <button onClick={() => setShowMap(!showMap)} className="w-full mb-8 py-6 bg-white dark:bg-white/5 border border-black/5 rounded-[2.5rem] font-black uppercase tracking-[0.4em] flex items-center justify-center gap-2 transition-all hover:bg-slate-50 dark:text-white group">
            <MapIcon size={20} className="text-[#8b57ff] group-hover:rotate-12 transition-transform" /> {showMap ? t.hideMap : t.showMap}
          </button>
          ...
          {/* TIMEFRAME SWITCHER */}
          <div className="flex justify-center mb-16 overflow-x-auto py-2 no-scrollbar">
             <div className="bg-white dark:bg-white/5 p-2 rounded-full border border-black/5 flex gap-1 shadow-sm">
               {[6, 12, 24, 48, 72, 120, 168, 240].map(v => (
                 <button key={v} onClick={() => setTimeframe(v)} className={`px-6 py-3 rounded-full text-[10px] font-black uppercase transition-all ${timeframe === v ? 'bg-[#8b57ff] text-white shadow-lg' : 'hover:bg-slate-100 dark:hover:bg-white/10 dark:text-white'}`}>
                   {v < 24 ? `${v}h` : `${v/24}d`}
                 </button>
               ))}
             </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {resorts.map((resort) => {
              ...
              resort.hourly?.precipitation?.slice(0, timeframe).forEach((p:number, i:number) => {
                totalPrec += p;
                if (p > 0 && resort.hourly.temperature_2m[i] <= 1) snow += p * 1.5; // Matematika korigovana na x1.5
              });
              const rain = Math.max(0, totalPrec - (snow / 1.5));
              const s = getStatus(snow);

              return (
                <Link key={resort.id} href={`/resort/${resort.id}`} className="group bg-white dark:bg-white/5 p-8 rounded-[3.5rem] border border-black/5 hover:shadow-2xl transition-all duration-500 block relative overflow-hidden">
                  
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest text-white mb-6 relative z-10" style={{ backgroundColor: s.color }}>
                    {s.icon} {s.txt}
                  </div>

                  <div className="mb-8 relative z-10">
                    <h3 className="text-3xl font-black uppercase tracking-tighter text-slate-900 dark:text-white group-hover:text-[#8b57ff] transition-colors">{resort.name}</h3>
                    <p className="text-[9px] font-black uppercase tracking-[0.4em] opacity-30 dark:text-white">{resort.country}</p>
                  </div>
                  
                  <div className="h-64 rounded-[3rem] p-8 flex flex-col justify-between text-white relative overflow-hidden shadow-xl z-10" style={{ backgroundColor: s.color }}>
                  ...
                  <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-20 dark:text-white mt-10 mb-5 text-center">{t.conditions}</p>
                  ...
                  <div className="grid grid-cols-3 gap-3 relative z-10">
                    ...
                    <div className="aspect-square bg-slate-50 dark:bg-white/5 rounded-[2rem] flex flex-col items-center justify-center border border-black/5 gap-2">
                       <Thermometer size={24} className="text-[#8b57ff] opacity-60" />
                       <span className="text-xl font-black dark:text-white leading-none">{resort.current?.temperature_2m.toFixed(0)}°</span>
                    </div>

                    <div className="aspect-square bg-slate-50 dark:bg-white/5 rounded-[2rem] flex flex-col items-center justify-center border border-black/5 gap-2">
                       <Navigation size={24} fill="currentColor" style={{ transform: `rotate(${resort.current?.wind_direction_10m}deg)` }} className="text-[#8b57ff] opacity-60" />
